...
- **方向分类 + 特征增强**：分类器直接输出 `P(价差>0)`；特征含节点间联动、D-1 日内形态、14/30 日滚动、负荷日均值。
...
| 指标 | 值 |
|---|---|
| 价差方向准确率（整体） | **62.6%**（优于随机 50%） |
| 方向准确率（实际交易时段） | **67.8%**（val 期 69.3%） |
| 决策占比 | sell ~31% / hold ~69%（单边，无买） |
| 模拟套利收益 | **val +5074 / test +1744**（两时段均盈利） |
| 最大单笔亏损 | **-162**（单边大幅压降风险，原 -2251） |
| spread q50 MAE / RMSE | 109.1 / 251.5 |

## 已知局限（如实说明）
1. **套利利润偏薄**：保守单边策略下 val +5074 / test +1744（日均 ~27–60），相对价差波动量级不大——价差本身难预测，方向 62.6% 带来的收益空间有限。
2. **单边牺牲双边机会**：只做"卖"（正价差）方向，负价差方向的套利机会（可能很大）被放弃。
3. **交易集中在 SNLNDRO**：单边策略下 CONTROLX 几乎不交易（模型对其正价差信号弱），实际交易大多落在 SNLNDRO。
4. **跨时段仍有漂移**：val/test 收益均为正但绝对值不同（5074 vs 1744），市场状态变化仍影响表现。
