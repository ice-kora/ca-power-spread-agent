...
给交易员/审计看，不给模型看。内容分五块：
  ...
  4. Risk Gate 结果（风控）  引用已校准规则（R7a/R6/R2/R4 警告），不重复发明
  5. 主要风险 + 人工确认     交易员最终把关
...
class DecisionCard:
    ...
    hour: int = -1
    model_source: str = "rule"   # 预测来源（rule / interpretable / catboost / committee）
